...
‎          console.log(`[\x1b[35ELANGFLOOD\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
‎        })
‎        .catch((error) => {
‎          if (error.message === 'Request timed out') {
‎            console.log(`[\x1b[35ELANGFLOOD\x1b[0m] ${getCurrentTime()} Request Timed Out`);
‎          } else if (error.response) {
‎            const extractedTitle = getTitleFromHTML(error.response.data);
‎            console.log(`[\x1b[35ELANGFLOOD\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
‎          } else {
‎            console.log(`[\x1b[ELANGFLOOD\x1b[0m] ${getCurrentTime()} ${error.message}`);
‎          }
‎        });
‎    }
‎
